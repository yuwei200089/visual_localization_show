...
class DrawPicture():
    def __init__(self):
        ...
        ax_width_inch = 12.4 - 4.25
        fig_dpi = 100
        # print(f"dpi {fig_dpi}, inch {ax_width_inch}")
        self.fig_width_px = ax_width_inch * fig_dpi

    def DrawEvent(self, data_event=[], i=0, multi_file=False):
        if multi_file:
            self.ax[i].eventplot(data_event, lineoffsets=0, linewidths=self.points_per_x_unit, linelengths=0.5)
            self.ax[i].eventplot([0, count[i]], lineoffsets=0, linewidths=3, linelengths=1, colors=["black"])
        else :
            self.ax.eventplot(data_event, lineoffsets=0, linewidths=self.points_per_x_unit, linelengths=0.5)
    ...
    def Show(self, data_event=[], percent=0, i=0, multi_file=False):
        self.points_per_x_unit = self.fig_width_px / count[i]
        self.DrawEvent(data_event, i, multi_file)
        ...
        if multi_file:
            ...
            self.ax[i].set_position([pos.bounds[0]+0.03, pos.bounds[1], pos.bounds[2], pos.bounds[3]])
            ...
            self.ax.set_position([pos.bounds[0]+0.03, pos.bounds[1], pos.bounds[2], pos.bounds[3]])
            self.ax.text(-0.08, 0.49, s, fontsize=19, verticalalignment='center', horizontalalignment='center', transform=self.ax.transAxes)
            self.ax.text(1.02, 0.49, f'{percent} %', fontsize=19, verticalalignment='center', horizontalalignment='center', transform=self.ax.transAxes)

if __name__ == '__main__':
    load_file_path = filedialog.askopenfilenames()
    ...
    count = []
    location_count = []
    picture_text = {'RealSense':'2 x RealSense D455', 'ZED':'ZED2'} 
    if all([1 if path.exists(j) else 0 for j in load_file_path]) and len(load_file_path) <= 2:
